...
function Api([string] $method, [string] $path, [object] $body, [string] $token, [string] $workspaceId, [string] $idempotencyKey, [string] $ifMatch = '') {
...
function Set-Environment {
    ...
    @('access.read','workspace.context.resolve','contacts.read','contacts.create','customers.view','customers.onboard_existing','customers.archive') | ForEach-Object -Begin { $i=0 } -Process { Set-Item "env:AccessControl__DevelopmentBootstrap__Capabilities__$i" $_; $i++ }
...
try {
    ...
    foreach ($fixture in @(
        ...
        @{ Name='Health Archived'; Suffix='archived' }
    )) {
    ...
    $recent = $customers | Where-Object Suffix -eq 'recent'; $overdue = $customers | Where-Object Suffix -eq 'overdue'; $unknown = $customers | Where-Object Suffix -eq 'unknown'; $archived = $customers | Where-Object Suffix -eq 'archived'
...
"@ | Out-Null
    $archivedResult = Api 'POST' "/customers/$($archived.CustomerId)/archive" @{} $token $workspaceId 'browser-customer-archived-archive' ('"' + $archived.Version + '"')
    $archived.Version = $archivedResult.result.version
...
"@ | Out-Null
    ...
    $env:UNICORECRM_TEST_ARCHIVED_CUSTOMER_ID=$archived.CustomerId; $env:UNICORECRM_TEST_ARCHIVED_CUSTOMER_CODE=$archived.CustomerCode
    ...
    Write-Output "CONNECTED CUSTOMER HEALTH BROWSER E2E: PASS (unknown=$($unknown.CustomerId), recent=$($recent.CustomerId), overdue=$($overdue.CustomerId), archived=$($archived.CustomerId), archivedNoFallback=true, noMutation=true)"
}
finally {
