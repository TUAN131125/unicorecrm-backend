...
try {
    ...
    foreach ($blocked in @('contacts.export', 'organizations.create', 'organizations.update', 'customers.onboard_existing', 'customers.edit', 'payments.customer_credit.allocate')) {
    ...
    Add-Result 'BLOCKED rows remaining in the matrix' 6 (@($matrix.capabilities | Where-Object { $_.admittedStatus -ceq 'BLOCKED' }).Count)
    ...
    $replaceBlocked = Invoke-ReplaceRole $replaceRoleId (New-ReplaceBody 'Replaceable Product Role' @('contacts.export')) '"2"'
