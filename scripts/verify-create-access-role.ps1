...
try {
    ...
    $happyBody = New-Body "  $([char]0x2003)Custom Managers$([char]0x2003)  " @('tasks.read','access.read','contacts.create','contacts.update') @(
    ...
    Add-Result 'exact capabilities persisted' 'access.read,contacts.create,contacts.update,tasks.read' ((Invoke-Sql "SELECT Capability FROM access.RoleCapabilities WHERE RoleId='$roleId' ORDER BY Capability").Capability -join ',')
    ...
    Assert-Rejected 'blocked capability' (New-Body 'Blocked Capability' @('contacts.delete')) 422 'VALIDATION_FAILED'
