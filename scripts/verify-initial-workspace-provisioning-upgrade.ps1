...
$initialCapabilities = @('contacts.create', 'contacts.delete', 'contacts.read', 'contacts.update') + $preContactsCapabilities
...
function Assert-SingleState($state, [string] $label) {
    ...
    Assert-True ([int](Invoke-SqlScalar "SELECT COUNT(*) FROM access.RoleCapabilities WHERE RoleId=(SELECT RoleId FROM access.Roles WHERE WorkspaceId='$($state.WorkspaceId)' AND Name='Workspace Owner') AND Capability='contacts.delete';") -eq 1) "$label contacts.delete exists exactly once"
...
try {
    ...
    $unexpectedCapabilities = @($preContactsCapabilities + 'contacts.export' | Sort-Object)
    ...
    Assert-True ((Invoke-SqlScalar "SELECT COUNT(*) FROM access.RoleCapabilities WHERE RoleId='$($stateF.RoleId)' AND Capability='contacts.export';") -eq '1') 'F: unexpected extra capability is not silently deleted or reclassified'
